    ...
    <>
      ...
      >
        ...
        >
          {/* Brand Logo */}
          <motion.div 
            className="mb-8 flex items-center justify-start w-full px-2"
            whileHover={{ scale: 1.02 }}
          >
            <motion.div
              whileHover={{ rotate: 5 }}
              transition={{ duration: 0.3 }}
              className="w-12 h-12 bg-gradient-to-br from-violet-600 to-fuchsia-600 rounded-xl flex items-center justify-center shadow-lg"
            >
              ...
              >
                ...
                <AnimatePresence>
                  {isExpanded && (
                    <motion.span 
                      ...
                      animate={{ opacity: 1, x: 0 }}
                      exit={{ opacity: 0, x: -10 }}
                      className="text-sm ml-3 font-medium whitespace-nowrap"
                    >
                      {item.label}
                    </motion.span>
                  )}
                ...
                {activeTab === item.id && (
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    className={`${isExpanded ? 'ml-auto' : 'hidden'} w-2 h-2 bg-gradient-to-r from-violet-400 to-fuchsia-400 rounded-full`}
                  />
                )}
                ...
                <AnimatePresence>
                  {!isExpanded && showTooltip === item.id && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, x: 10 }}
                      className="absolute left-full ml-2 px-2 py-1 bg-gray-800 text-white text-xs rounded-md whitespace-nowrap z-50"
                    >
                      {item.label}
                      {item.badge && (
                        <span className="ml-2 bg-red-500 text-white text-xs rounded-full h-4 w-4 flex items-center justify-center inline-block">
                          {item.badge}
                        </span>
                      )}
                    </motion.div>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className={`mt-8 py-3 bg-gradient-to-r from-violet-600 to-fuchsia-600 rounded-xl text-white font-medium flex items-center relative overflow-hidden group ${isExpanded ? 'w-full px-4' : 'w-12 justify-center'}`}
          >
          ...
          <div className="mt-auto pt-6 border-t border-gray-800 w-full">
            <motion.button
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              className={`flex items-center w-full p-3 rounded-xl bg-gray-800/30 hover:bg-white/5 transition-colors ${isExpanded ? 'justify-start' : 'justify-center'}`}
        ...
        <motion.button
          ...
          className="self-center mt-4 w-6 h-12 bg-gray-800 rounded-r-lg flex items-center justify-center text-gray-400 hover:text-white"
        >
